...
<body>
	...
	<form action="${pageContext.request.contextPath }/insertQuestionnaire.action" method="post">
		<div>
			<div>
				<input type="text" name="designTempletName" value=${designTempletName } id="designTempletName" placeholder="请输入字典模板名称">
			</div>
			<br />
			<br />
			<br />
			...
			<div>
					...
					<table border="1px" align="center" id="table_single">
						...
						<c:forEach varStatus="i" var="designSingle" items="${designSingleList }">
							<tr id="single${i.count }">
								<td>
									${i.count }<textarea name="single_question" id="single_question_${i.count }" cols="40" rows="5">${designSingle.designSingleContent }</textarea>
								</td>
								<%
									n = 65;
								%>
									...
									<c:forEach var="designSingleOption" items="${designSingleOptionList }">
										<c:if test="${designSingle.designSingleId == designSingleOption.designSingleId }">
											<%
												temp = (char)n++;
											%>
											<%=temp %>.<textarea name="single_answer_<%=temp %>" id="single_answer_${i.count }_<%=temp %>" cols="70" rows="1">${designSingleOption.designSingleOptionContent }</textarea>
											<br />
										</c:if>
									</c:forEach>
									<%
				...
				<div>
					<table border="1px" align="center" id="table_check">
						...
						<c:forEach varStatus="i" var="designCheckbox" items="${designCheckboxList }">
							<tr id="check${i.count }">
								<td>
									${i.count }<textarea name="check_question" id="check_question_${i.count }" cols="40" rows="5">${designCheckbox.designCheckboxContent }</textarea>
								</td>
								<%
									n = 65;
								%>
									...
									<c:forEach var="designCheckboxOption" items="${designCheckboxOptionList }">
										<c:if test="${designCheckbox.designCheckboxId == designCheckboxOption.designCheckboxId }">
											<%
												temp = (char)n++;
											%>
											<%=temp %>.<textarea name="check_answer_<%=temp %>" id="check_answer_${i.count }_<%=temp %>" cols="70" rows="1">${designCheckboxOption.designCheckboxOptionContent }</textarea>
											<br />
										</c:if>
									</c:forEach>
									<%
				...
				<div>
					<table border="1px" align="center" id="table_text">
						...
						<c:forEach varStatus="i" var="designText" items="${designTextList }">
							<tr id="text${i.count }">
								<td>
									${i.count }<textarea name="text_question" id="text_question_${i.count }" cols="110" rows="7">${designText.designTextContent }</textarea>
								</td>
								<td>
...
<script type="text/javascript">
	...
	/* 增加的单选题、多选题、文本题序号 */
	var i = ${designSingleList.size() + 1 };
	var j = ${designCheckboxList.size() + 1 };
	var k = ${designTextList.size() + 1 };
	/* 记录被删除的题数，验证提交时使用 */
	var a = 0;
	var b = 0;
	var c = 0;
	...
	$("#submit").click(function() {
		if($("#designTempletName").val().length == 0) {
			alert("请填写字典模板名称");
			return false;
